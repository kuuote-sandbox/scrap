    ...
    switch (n[0]) {
      ...
      case "[":
        ...
        if (linkBody.includes("://")) {
          ...
          // case wiki link
          return (
            <a href={linkBody} onClick={(e) => e.stopPropagation()}>
              {linkBody}
            </a>
          );
        }
  ...
  function buildLine(indent) {
    ...
    return (
      <span
        ...
        style={{ width: String(props.line.indent * 1.5) + "em" }}
      >
...
class Article extends React.Component {
  ...
  doEnter() {
    ...
    newLines.splice(
      index + 1,
      0,
      parseLine(" ".repeat(newLines[index].indent)),
    );
  ...
  doTab(shiftKey) {
    ...
    if (
      (!shiftKey && cursor === input.value.length &&
        this.state.cursor < this.state.lines.length - 1) ||
  ...
  postMessage() {
    ...
    for (i = 0; i < len; i++) {
      if (
        !(lines[i].indent === origin[i].indent &&
          lines[i].text === origin[i].text)
      ) {
        break;
      }
    }
    ...
    switch (lines.length - origin.length) {
      ...
      case 0: // replace
        ...
        msg = {
          "command": "replace",
          "line": i,
          "text": "\t".repeat(lines[i].indent) + lines[i].text,
        };
        console.log("replace");
        break;
      case 1:
